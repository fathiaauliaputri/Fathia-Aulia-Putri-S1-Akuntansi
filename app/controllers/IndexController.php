...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		if(!empty($user)){
			...
			$password_hash = $user[''];
			$this->modeldata[''] = $password_hash; //update the modeldata with the password hash
			if(password_verify($password_text,$password_hash)){
        		unset($user['']); //Remove user password. No need to store it in the session
	...
	function register($formdata = null){
		if($formdata){
			...
			if($this->validated()){
				...
				if($rec_id){
					$this->login_user($modeldata[''] , $password_text);
					return;
				}
				else{
					$this->set_page_error();
